...
export default function LoginPage() {
  ...
  const [error, setError] = useState("");
   ...
   const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/login", {  // link ya Laravel API
        method: "POST",
        headers: {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#001F3F] via-[#002B5B] to-[#001F3F] px-4 mb-25 -mt-10">
      <div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-10 shadow-2xl">
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-5">
          ...
          <button
            type="submit"
            className="mt-4 bg-gradient-to-r from-cyan-500 to-teal-400 text-white font-bold py-3 rounded-lg shadow-lg cursor-pointer hover:scale-105 transition-transform"
          >
            Sign In
          </button>
        </form>
